...
##' @author Getúlio Caixeta Ferreira, \email{getulio.caifer@@gmail.com}
##' @keywords rqtl
##' @examples
##'
##' \dontrun{
...
##'  seq2<-make_seq(order_seq(input.seq= make_seq(lg,2),twopt.alg = "rcd"), "force")
##'  seq3<-make_seq(order_seq(input.seq= make_seq(lg,3),twopt.alg = "rcd"), "force")
##'  draw_map2(seq1,seq2,seq3,data=onemap_example_f2,tag = "all",group.names = c("Chr 1","Chr 2","Chr 3"))
##'
##' }
##'@export
draw_map2<-function(...,data,tag=NULL,id=T,pos =T,cex.label=NULL,main=NULL,group.names=NULL,centred=F,y.axis=T,space=NULL,col.group=NULL,col.mark=NULL,col.tag=NULL,output=NULL){
  ...
  # Preparing plot area
  if(strsplit(output,".",T)[[1]][length(strsplit(output,".",T)[[1]])]=="bmp"){
    bmp(output,height = 15,width = nchr*(1+space)+3,res = 300,units = "cm")
  } else  if(strsplit(output,".",T)[[1]][length(strsplit(output,".",T)[[1]])]=="jpeg"){
    jpeg(output,height = 15,width = nchr*(1+space)+3,res = 300,units = "cm")
  } else if(strsplit(output,".",T)[[1]][length(strsplit(output,".",T)[[1]])]=="png"){
    png(output,height = 15,width = nchr*(1+space)+3,res = 300,units = "cm")
  } else if(strsplit(output,".",T)[[1]][length(strsplit(output,".",T)[[1]])]=="tiff"){
    tiff(output,height = 15,width = nchr*(1+space)+3,res = 300,units = "cm")
  } else if(strsplit(output,".",T)[[1]][length(strsplit(output,".",T)[[1]])]=="pdf"){
    pdf(output,height = 15/2.54,width = (nchr*(1+space)+3)/2.54)
  } else if(strsplit(output,".",T)[[1]][length(strsplit(output,".",T)[[1]])]=="eps"){
    postscript(output,height = 15/2.54,width = (nchr*(1+space)+3)/2.54,paper = "special",horizontal = F,onefile = F)
  } else {
    output<-paste(output,"eps",sep = ".")
    postscript(output,height = 15/2.54,width = (nchr*(1+space)+3)/2.54,paper = "special",horizontal = F,onefile = F)
    }
