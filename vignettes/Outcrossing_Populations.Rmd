...
all_progeny_haplot <- progeny_haplotypes(groups_list, 
                                         most_likely = FALSE, 
                                         ind = "all")
...
# Save heavy OneMap sequences objects

Each OneMap 3.0 sequences object contains all data input. These internal objects do not occupy extra space in R environment, but they can inflate the size of saved objects as RData. If you wish to save OneMap RData objects, use the following function to reduce the file size:

```{r, eval=FALSE}
groups_list <- list(CHR1_final,CHR2_final, CHR3_final)

save(groups_list, file = "groups_list_large.RData") # 371kb

save_onemap_sequences(groups_list, filename = "groups_list.RData") # 181.9 kb
```

Use `load_onemap_sequences` to read it back to R environment:

```{r, eval=FALSE}
# load list of sequences
groups_list <- load_onemap_sequences("groups_list.RData") 

# recover the onemap data object:
bins_example <- groups_list[[1]]$data.name

# recover twopoints estimations
twopts <- groups_list[[1]]$twopt
